...
const App = () => {  
  ...
  const addTodo = async (todoText) => {  
    try {  
      ...
      const response = await axios.post('http://localhost:8080/api/todos', { title: todoText }, {  
    headers: {  
  ...
  const editTodo = async (id, newText) => {  
        try {  
      const response = await axios.put(`http://localhost:8080/api/todos/${id}`, { title: newText });  
